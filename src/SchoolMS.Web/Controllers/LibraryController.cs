using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;
using SchoolMS.Application.DTOs;
using SchoolMS.Application.Interfaces;
using SchoolMS.Web.Filters;

namespace SchoolMS.Web.Controllers;

[Authorize]
public class LibraryController : Controller
{
    private readonly ILibraryService _service;
    private readonly IBranchService _branchService;
    private readonly IPlatformService _platformService;
    private readonly IOneSignalNotificationService _pushService;

    public LibraryController(ILibraryService service, IBranchService branchService, IPlatformService platformService,
        IOneSignalNotificationService pushService)
    { _service = service; _branchService = branchService; _platformService = platformService; _pushService = pushService; }

    private bool IsSuperAdmin => User.IsInRole("SuperAdmin");
    private int? CurrentSchoolId { get { var c = User.FindFirst("SchoolId"); return c != null && int.TryParse(c.Value, out var id) ? id : null; } }

    [HasPermission("Library", "View")]
    public async Task<IActionResult> Index()
    {
        ViewData["Title"] = "Library Management";
        ViewBag.IsSuperAdmin = IsSuperAdmin;
        ViewBag.Borrows = await _service.GetAllBorrowsAsync();
        if (IsSuperAdmin) { ViewBag.Schools = await _platformService.GetAllSchoolsAsync(); ViewBag.Branches = await _branchService.GetAllAsync(); }
        else { ViewBag.Schools = new List<SchoolDto>(); ViewBag.Branches = CurrentSchoolId.HasValue ? await _branchService.GetBySchoolIdAsync(CurrentSchoolId.Value) : new List<BranchDto>(); }
        return View(await _service.GetAllBooksAsync());
    }

    [HasPermission("Library", "Add")]
    public async Task<IActionResult> Create()
    {
        ViewData["Title"] = "Add Book";
        await LoadViewBags();
        return View();
    }

    [HttpPost, HasPermission("Library", "Add"), ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(LibraryBookDto dto)
    {
        if (!IsSuperAdmin && CurrentSchoolId.HasValue) dto.SchoolId = CurrentSchoolId.Value;
        await _service.CreateBookAsync(dto);
        await _pushService.SendToPersonTypesAsync("New Book Added",
            $"{dto.Title} by {dto.Author ?? "Unknown"} is now available",
            new[] { "Student", "Teacher" }, dto.SchoolId);
        return RedirectToAction(nameof(Index));
    }

    [HasPermission("Library", "Edit")]
    public async Task<IActionResult> Edit(int id)
    {
        var item = await _service.GetBookByIdAsync(id); if (item == null) return NotFound();
        ViewData["Title"] = "Edit Book";
        await LoadViewBags(item.SchoolId);
        return View("Create", item);
    }

    [HttpPost, HasPermission("Library", "Edit"), ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(LibraryBookDto dto)
    {
        if (!IsSuperAdmin && CurrentSchoolId.HasValue) dto.SchoolId = CurrentSchoolId.Value;
        await _service.UpdateBookAsync(dto);
        await _pushService.SendToPersonTypesAsync("Library Book Updated",
            $"{dto.Title} has been updated",
            new[] { "Student", "Teacher" }, dto.SchoolId);
        return RedirectToAction(nameof(Index));
    }

    [HttpDelete("Book/{id}"), HasPermission("Library", "Delete"), ValidateAntiForgeryToken]
    public async Task<IActionResult> DeleteBook(int id) { await _service.DeleteBookAsync(id); return Ok(); }

    [HttpPost, HasPermission("Library", "Add"), ValidateAntiForgeryToken]
    public async Task<IActionResult> Borrow([FromBody] BookBorrowDto dto) => Ok(await _service.BorrowBookAsync(dto));

    [HttpPost, HasPermission("Library", "Edit"), ValidateAntiForgeryToken]
    public async Task<IActionResult> Return(int borrowId) { await _service.ReturnBookAsync(borrowId); return Ok(); }

    [HttpGet]
    public async Task<IActionResult> GetBranchesBySchool(int schoolId)
    {
        var branches = await _branchService.GetBySchoolIdAsync(schoolId);
        return Json(branches.Select(b => new { b.Id, b.Name }));
    }

    private async Task LoadViewBags(int? editSchoolId = null)
    {
        ViewBag.IsSuperAdmin = IsSuperAdmin;
        ViewBag.Schools = IsSuperAdmin ? await _platformService.GetAllSchoolsAsync() : new List<SchoolDto>();
        if (IsSuperAdmin)
            ViewBag.Branches = editSchoolId.HasValue && editSchoolId.Value > 0 ? await _branchService.GetBySchoolIdAsync(editSchoolId.Value) : new List<BranchDto>();
        else
            ViewBag.Branches = CurrentSchoolId.HasValue ? await _branchService.GetBySchoolIdAsync(CurrentSchoolId.Value) : new List<BranchDto>();
    }
}
