...
public class VisitorsController : Controller
{
    ...
    public VisitorsController(IVisitorService service, IBranchService branchService, IPlatformService platformService,
    ...
    [HttpPost, HasPermission("Visitors", "Add"), ValidateAntiForgeryToken]
    public async Task<IActionResult> Create(VisitorDto dto)
    {
        if (!IsSuperAdmin && CurrentSchoolId.HasValue) dto.SchoolId = CurrentSchoolId.Value;
        await _service.CheckInAsync(dto);
        await _pushService.SendToPersonTypesAsync("Visitor Check-In",
            $"{dto.VisitorName} - Purpose: {dto.Purpose}",
            new[] { "Staff" }, dto.SchoolId);
        return RedirectToAction(nameof(Index));
    }

    [HttpPost, HasPermission("Visitors", "Edit"), ValidateAntiForgeryToken]
    public async Task<IActionResult> CheckOut(int id) { await _service.CheckOutAsync(id); return Ok(); }
